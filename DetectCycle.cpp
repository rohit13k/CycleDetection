...
bool
allPathWithoutCandidate(nodeid w, nodeid rootnode, long t_s, long t_e, vector<std::string> path_till_here,
                        std::set<std::string> seen_node, set<int> *cycleFound) {
    ct[w] = t_s;
    long lastp = 0;
    set<pedge> E;
    ...
    for (auto x: V) {
        ...
        while (!time_x.empty()) {
            t_min = *time_x.begin();
            set<string> new_seen_node = seen_node;
